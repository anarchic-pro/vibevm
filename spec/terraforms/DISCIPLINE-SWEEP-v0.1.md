...
- **Gating:** 16 crates gated / 4 exempt-with-reason / 1 under `pub-doctest` (`vibe-core`). *(The count comes from the `CONFORM_GATED` const itself, read by the collector — never from memory. An LLM inventorying this during authoring miscounted it as 18; the const says 16. This is the "instrument discipline — count the list, not the record" lesson, SHRINK v0.1 §0, made mechanical.)*
...
- **1b · Widen the gate for free (`summary.pub_doctest_promotion_candidates`).** A gated crate at **0 type-coverage gap** is ready to enter `GATED_PUB_DOCTEST` (`xtask/src/conform.rs`) with zero drain. Add the crate to the const, run `cargo xtask conform check` — expect **0 new** (the collector predicted it; the gate confirms it). If a finding does appear, the collector slightly under-counted; drain that one type and proceed. This is the cheapest ratchet win there is. *(At authoring: `conform-core`, `conform-frontend-rust`, `env-audit`, `specmark-grammar` are all ready.)*
