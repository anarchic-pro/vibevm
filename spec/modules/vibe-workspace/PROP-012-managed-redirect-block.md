...
**Status:** DRAFT — requirements captured in an owner discussion on 2026-05-22 during M1.18 Phase-7 planning; the small §5 open-question set wants an owner decision (chiefly the marker syntax) before implementation.
...
**Decision.** The block is delimited by a literal opening marker `<vibevm>` and closing marker `</vibevm>`, each written alone on its own line. The canonical block vibevm writes:

```
<vibevm>
<!-- Generated by vibe — do not edit inside this block. It is rewritten on `vibe install`. -->
... the boot redirect (§2.5) ...
</vibevm>
```
...
1. **Marker syntax.** `<vibevm>` … `</vibevm>` literal tags (the proposal — short, readable, unmistakably vibevm's) versus HTML-comment delimiters `<!-- vibevm:begin -->` … `<!-- vibevm:end -->` (invisible to every markdown renderer, the conventional managed-block idiom, no risk a renderer mangles a bare non-standard tag). Both are equally machine-locatable by a line scan. The literal form is the working default; it deserves a moment's owner confirmation because it becomes effectively permanent once projects carry it.
2. **Exit code.** Reuse `3` (package conflict — a malformed managed block is conflict-shaped) or mint a dedicated code? Leaning: reuse `3`.
3. **Old-redirect detection for migration (§4).** Match the exact Phase-4 generated-header string (precise; a false positive would wrongly destroy a hand-authored file) versus a looser heuristic. Leaning: exact string match.
