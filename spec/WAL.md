...
- **Priority-walk discriminator over-strict on primary connect-failure** (surfaced 2026-05-04 by `manual-tests/M1.6-mirror-vendor-smoke.md` Scenario B4). When the primary `[[registry]]` URL points at a dead host, `git ls-remote --tags` returns `fatal: unable to access '…': Failed to connect to … port 443 …: Could not connect to server`. Two layers conspire: `crates/vibe-registry/src/git_backend/shell.rs::classify_clone_error` matches `could not resolve host` / `could not read from remote repository` / `network is unreachable` but not `failed to connect` / `could not connect to server`, so the failure lands as `GitError::CommandFailed` rather than `NetworkUnreachable`; and `MultiRegistryResolver` translates only `RepoNotFound` into the `UnknownPackage` shape that the priority-walk falls through on (`git_package_registry.rs:411`), so even a properly-classified `NetworkUnreachable` would still halt install rather than falling through. Two-part fix needed: (a) widen the classifier substring set, (b) decide whether transient connect-failure on the primary should fall through or fail-fast (the smoke author's intent reads as "fall through" — see Scenario B4 expected text — but that is a policy decision worth confirming before we silently skip a misconfigured primary). The functional Scenario B path (B1–B3, primary returns `UnknownPackage`) is unaffected.
