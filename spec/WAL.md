...
Workspace state: ~225 tests across the workspace (8 new in `vibe-registry::git_package_registry::tests` — `fetch_falls_through_to_mirror_when_primary_unreachable`, `fetch_prefers_primary_when_both_reachable`, `fetch_falls_through_when_primary_update_fails` [exercises the wipe-and-rebootstrap branch], `fetch_with_expected_hash_passes_through_when_no_pin`, `fetch_with_expected_hash_skips_mirror_with_disagreeing_content` [primary unreachable → mirror[0] serves divergent content, hash mismatch, fall through → mirror[1] serves canonical content matching pin, accepted], `fetch_with_expected_hash_returns_last_attempt_when_no_match`, `refresh_package_falls_through_to_mirror_when_primary_unreachable`, `fetch_dep_manifest_clone_fallback_uses_mirror_dispatch`). The `FakeBackend` test fixture gained a per-URL `bootstrap_urls` log, `fail_update_for_url` injection (writes the origin URL into `.git/origin-url` so `update` can selectively fail per origin), and a `NoArchiveBackend` wrapper for the clone-fallback test. `cargo clippy --workspace --all-targets -- -D warnings` clean.

Phase B v0 cache-mutating slice is now complete. Mirrors are useful for actual installs, not just dep walks. Next: `vibe vendor` (offline mirror generator), then exercise the multi-registry surface against a real second `[[registry]]` and a real `[[mirror]]` URL in a manual smoke test.
