...
- **Mirror-walk classifier-aware fall-through pending.** Per [PROP-002 §2.3.1](modules/vibe-registry/PROP-002-decentralized-registry.md#failure-discriminator), `[[mirror]]` should absorb any availability failure (`NetworkUnreachable`, `AuthFailed`, `CommandFailed`, mirror-side `RepoNotFound`) and fall through to the next mirror; today the cache-mutating mirror walk in `GitPackageRegistry::fetch` / `refresh_package` already iterates mirrors-then-primary and treats every per-source failure as a fall-through, but the discrimination is by-position (any source failure → next source) rather than by-error-variant. With the M1.6 Phase B v0 surface that is functionally equivalent (no caller distinguishes mirror failure modes today), but the PROP makes the policy explicit so the implementation can be tightened in the next slice without surprising any consumer.
